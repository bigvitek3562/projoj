<?php
  ...
  $queryString = "select recipes.id, recipes.title, recipes.description from recipes join recipes_ingredients on recipes_ingredients.recipe_id = recipes.id where recipes_ingredients.ingredient_id IN (".$searchTerm.") group by recipes.id, recipes.title, recipes.description having count(recipes_ingredients.ingredient_id) = 2";
  ...
  if(mysqli_num_rows($query)>0){
    while ($row = mysqli_fetch_array($query)){
		$output .= '<div class="recipe_link"><a target="_blank" rel="noopener noreferrer" style="font-size: 30px; color:white; text-decoration: none;"
      ">'.$row['title'].'</a><p style="margin: 1px;">'.$row['description'].'</p></div>';
    }
  }else $output = 'Мы пока не знаем рецептов с такими ингредиентами...';
